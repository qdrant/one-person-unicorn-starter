...
async def main():
    ...
    qdrant_memory_agent = mistral_client.beta.agents.create(model=MISTRAL_MODEL, description=AGENT_DESCRIPTION, instructions=AGENT_INSTRUCTIONS, name=AGENT_NAME)
    ...
    async with RunContext(agent_id=qdrant_memory_agent.id, continue_on_fn_error=True) as run_ctx_1:
        ...
        store_result = await mistral_client.beta.conversations.run_async(run_ctx=run_ctx_1, inputs="Remember that the secret code for the hackathon is 'Aurora Penguin'.")
    ...
    async with RunContext(agent_id=qdrant_memory_agent.id, continue_on_fn_error=True) as run_ctx_2:
        ...
        recall_result = await mistral_client.beta.conversations.run_async(run_ctx=run_ctx_2, inputs="What is the secret code for the event?")
